# Acquisitions Module Database Schema

## Overview
Complete acquisition workflow from vendor management through purchase orders, invoicing, payments, budgeting, and contract management.

## Tables

### 1. `vendors`
Manage library material suppliers and service providers.
...
CREATE TABLE vendors (
  ...
  code VARCHAR(50) UNIQUE, -- Internal vendor code
...
### 2. `budgets`
Track library budgets and fund allocations.
...
CREATE TABLE budgets (
  ...
  -- Calculated: available = allocated - (spent + encumbered)

  -- Categorization
  category VARCHAR(100), -- Books, Serials, Digital, etc.
...
### 3. `acquisition_orders`
Purchase orders for library materials.
...
CREATE TABLE acquisition_orders (
  ...
  -- Status
  status VARCHAR(50) DEFAULT 'pending', -- pending, ordered, partial, received, cancelled, closed
...
CREATE TABLE order_items (
  ...
  marc_record_id UUID REFERENCES marc_records(id), -- Can be NULL if not yet cataloged
  ...
  -- Status
  status VARCHAR(50) DEFAULT 'ordered', -- ordered, received, cancelled, backordered
...
CREATE TABLE payments (
  ...
  -- Payment Method
  payment_method VARCHAR(100), -- Check, Wire Transfer, Credit Card, etc.
...
CREATE TABLE contracts (
  ...
  -- Type
  contract_type VARCHAR(100), -- Purchase, License, Subscription, Service
  resource_type VARCHAR(100), -- Database, Ebook Package, Journals, etc.
  ...
  currency VARCHAR(3) DEFAULT 'USD',
  payment_frequency VARCHAR(50), -- Annual, Monthly, One-time, etc.
  auto_renew BOOLEAN DEFAULT FALSE,

  -- Status
  status VARCHAR(50) DEFAULT 'active', -- draft, active, expiring, expired, cancelled
...
```sql
-- Enable RLS on all acquisitions tables
...
-- Authenticated users can manage acquisitions
...
```sql
-- Update budget spent/encumbered amounts when order items change
...
2. Run all SQL statements above in order
3. Verify tables are created in Table Editor
4. Test budget tracking triggers

## Future Enhancements

- Email alerts for contract renewals
- Automatic invoice matching with orders
